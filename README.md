...
- 📚 **Stories**: a library with several kids' retellings (currently *Fourth Wing*, *Dungeon Crawler Carl*, *Treasure Island* and *Red Rising*). Progress and high scores are per story; word lists and sticky words carry across.
...
| `stories/*.js` | The stories (kids' retellings of *Fourth Wing*, *Dungeon Crawler Carl*, *Treasure Island* and *Red Rising*) — each pushes `{ id, title, blurb, level, chapters: [{ title, text }] }` onto `window.STORIES` |
